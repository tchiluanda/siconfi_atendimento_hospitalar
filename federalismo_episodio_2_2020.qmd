...
g3<-dataset_analise %>%
  ...
  mutate(tipo_deslocamento = ifelse(deslocamento=="0","locais","De outros muncípios")) %>%
  mutate(nome_nivel_hierarquia = reorder(nome_nivel_hierarquia.x, quantidade_internacoes)) %>%
  ungroup() %>%
  bind_rows(
    dataset_analise %>%
      ...
      mutate(tipo_deslocamento = "De outros muncípios") %>%
      mutate(nome_nivel_hierarquia= reorder(nome_nivel_hierarquia.y, quantidade_internacoes)) %>%
      ungroup()
  ) %>%
  ggplot() +
...
Feita a apresentação dos gráficos fica fácil perceber que a conclusão mais importante da associação entre gastos e grupos de entrada e saída de pacientes. Quando se analisa as saídas de pacientes, no gráfico à esquerda, descobrimos que o grupo **saída forte** apresenta mediana de gastos proporcionais com hospitais muito inferior aos demais grupos. Além disso, o grupo **entrada forte** apresenta a mediana destacadamente superior aos demais grupos.

O gráfico do lado direito traz sinais contraditórios. Os grupos **entrada forte** e **saída forte** apresentam medianas parecidas e maiores do que as dos outros dois grupos. Nesse caso não é possível chegar-se a conclusões sobre o impacto da despesa com hospitais e atração de pacientes.
