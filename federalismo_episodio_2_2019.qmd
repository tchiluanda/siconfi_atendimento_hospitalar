...
dataset_analise <- readRDS("~/Github/siconfi_atendimento_hospitalar/dataset_analise_2019.RDS")
...
muni_sel_repel<-
   municipios_seat %>%
   mutate(code_muni = str_sub(as.character(code_muni),1,6)) %>%
   filter(code_muni %in% c("260960", "260790")) %>% #261160-Recife,260790 -Jaboatão, 260960 - Olinda, 260410 - Caruarau, 260545 - Fernando de Noronha, 110150 - Seringueiras-RO
  inner_join(muni_sel)
...
```{r fig.width=20, fig.height=10, fig.dpi=500}
agrupamento_municipio_cluster<-readRDS("agrupamento_municipio_2019.RDS")
