---
title: "Painel de gastos hospitalares"
output: 
...
###Dados
dataset_analise <- readRDS("dataset_analise_2020.RDS")
...
Fluxos de saída e entrada de pacientes
=====================================  
...
```{r}
selectInput(inputId = "regic_hierarquia_s", label= "Selecione uma ou mais hierariquias pra saída", choices= busca_hierarquia_regic(),selected = "Centro Local", multiple = TRUE )


selectInput(inputId = "regic_hierarquia_e", label= "Selecione uma ou mais hierariquias pra saída", choices= busca_hierarquia_regic(),selected = busca_hierarquia_regic(), multiple = TRUE )
...
dataset_analise %>%
  filter(deslocamento==1,
         nome_nivel_hierarquia.x %in% input$regic_hierarquia_s) %>%
 ...
 aluvial<-
     dataset_analise %>%
  filter(deslocamento==1,
         nome_nivel_hierarquia.x == input$regic_hierarquia_s,
         nome_nivel_hierarquia.y == input$regic_hierarquia_e) %>%
  mutate(saída = nome_nivel_hierarquia.x,
         entrada =nome_nivel_hierarquia.y,20) %>%
...
aluvial$entrada <- factor(aluvial$entrada, levels = unique(ordem_y$nome_nivel_hierarquia.y[order(ordem_y$quantidade)])) 
 ...
 p<- easyalluvial::alluvial_wide(aluvial, stratum_labels= FALSE)
...
Entradas e saídas de pacientes por cidade
=====================================  
...
```{r}
selectInput(inputId = "regic_mun_desloc", label= "Selecione um municipio", choices= busca_todos_municipios(),selected = "Recife", multiple = FALSE )
...
```{r}
selectInput(inputId = "regic_mun_gasto", label= "Selecione um ou mais municipios", choices= busca_todos_municipios(),selected = "Recife", multiple = TRUE )
...
# codigo_muni<-
#     unique(dataset_analise$codufmun [dataset_analise$mun_res_nome.y %in% input$regic_mun_gasto])

codigo_muni<-
    unique(dataset_analise$munic_res[str_to_lower(dataset_analise$mun_res_nome.x) %in% str_to_lower(input$regic_mun_gasto)])

print(input$regic_mun_gasto)
print(codigo_muni)
...
```{r}
selectInput(inputId = "regic_mun_gasto", label= "Selecione um ou mais municipios", choices= busca_municipio_regic(),selected = "Recife", multiple = TRUE )
