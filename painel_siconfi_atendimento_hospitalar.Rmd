...
```{r}
selectInput(inputId = "regic_mun_desloc", label= "Selecione um municipio", choices= busca_municipio_regic(),selected = "Recife", multiple = FALSE )
...
renderPlot({
  ...
  codigo_muni<-
    unique(dataset_analise$munic_res[dataset_analise$mun_res_nome.x==input$regic_mun_desloc])
...
dataset_analise %>%
  ...
  geom_text_repel(data= muni_sel_saida, aes(x=cluster_4_k, y=perc.x,label= mun_res_nome.x),color="white")+
...
dataset_analise %>%
  ...
  geom_text_repel(data= muni_sel_entrada, aes(x=cluster_4_k, y=perc.y,label= mun_res_nome.y), color="white", fontface = "bold",)+
  scale_fill_continuous_sequential(palette= "Red-Yellow")+
  theme_light() +
  theme(
    text = element_text(size=18),
