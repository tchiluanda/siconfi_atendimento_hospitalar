...
renderPlot({
  ...
  g1_saida<<-
    ...
    geom_sf(fill="white",size=1,pch=21, color="#444444")+
  ...
  g2_saida<<-
    ...
    geom_sf( aes(fill=quantidade),pch=21, color="#444444", size=1)+
...
renderPlot({
  ...
  g2_entrada<<-
    ...
    ggplot()+
    geom_sf( aes(fill=quantidade),pch=21, color="#444444", size=1, show.legend = TRUE)+
    geom_sf( data= muni_sel_foco, aes(size=quantidade),pch=21, color="#444444", fill="white")+
    geom_sf(data= regioes_saude, fill= NA, color = "#505050")+
    geom_sf(data = estados_mapa, fill=NA, color="lightgreen")+#505050
    geom_text_repel(data = muni_sel_repel,
...
renderDT({
  ...
  selecao<-
    dataset_analise %>%
    filter(deslocamento == 1,
           mun_res_nome.x == input$regic_mun_vis) %>% # antes havia esse filtro complementar: ,perc.x>=0,perc.x<=50
    distinct(nome_nivel_hierarquia.x,munic_res, uf.x,perc.x, mun_res_nome.x) %>%
    inner_join(
      agrupamento_municipio_cluster %>%
        rename(munic_res=code_muni)
...
renderDT({
  ...
  codigo_muni<-
    unique(dataset_analise$munic_res[dataset_analise$mun_res_nome.x %in% input$regic_mun_vis])
...
renderDT({
  ...
  if (NROW(unique(dataset_analise$codufmun[dataset_analise$mun_res_nome.y==input$regic_mun_vis])) == 0){
    return()
  }
...
renderDataTable({
  ...
  mun_selecionado<-input$regic_mun_vis
  
  selecao<-
    dataset_analise %>%
    filter(deslocamento == 1,
           mun_res_nome.x == mun_selecionado) %>% # antes havia esse filtro complementar: ,perc.x>=0,perc.x<=50
    distinct(nome_nivel_hierarquia.x,munic_res, uf.x,perc.x, mun_res_nome.x) %>%
    inner_join(
      agrupamento_municipio_cluster %>%
        rename(munic_res=code_muni)
  ...
  names(muni_sel)<-
    c("municipio_destino_paciente",
      ...
      paste("percentual_pacientes_deslocados_de_", mun_selecionado),
  ...
  if (NROW(unique(dataset_analise$codufmun[dataset_analise$mun_res_nome.y==mun_selecionado])) == 0){
    ...
    codigo_muni<-
      unique(dataset_analise$codufmun[dataset_analise$mun_res_nome.y==mun_selecionado])
    ...
    names(muni_sel)<-
      c("municipio_origem_paciente",
        ...
        paste("percentual_de_pacientes_deslocados_para_",mun_selecionado),
        "distancia_ao_municipio"
      )
